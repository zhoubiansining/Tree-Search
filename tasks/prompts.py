...
summary_prompt = '''
你的任务是给定一个理科问题和已有的解答步骤，按照规定格式用一句话概述最终答案。
下面是几个例子，请学习

输入:
给定问题: 求数列${n^{1/n}}$(n=1、2、3...为正整数)的最大项的值。
已有步骤:
...
输出:
综上所述，p 需要满足 $p > -1$（来自步骤2的结论）以及 $p < 3$（来自步骤3的结论），我们最终得出当 $-1 < p < 3$ 时，广义积分 $J$ 收敛。
...
输出:
综上所述，函数$f(x)$在R上的最大值为1，在$x=2$处取到。
...
下面是你要给出概述的题目，请按规定格式"综上所述，..."输出最终答案。

给定问题: 
'''

evaluate_summary_prompt = '''
你的任务是给定一个理科问题和已有的解答步骤，按固定格式输出最终答案，输出需转化为整数或者两位小数。
下面是几个例子，请学习
...
你的任务是给定一个理科问题，已有的解答步骤（并不是完整的答案）以及针对下一步的意见（提示），具体地给出完整的下一步。
假设已有步骤为n步，则输入的格式为:
”题目:...
已有步骤:
步骤1:...
...
其中...表示省略的输入信息。
如果n等于0，你需要按照意见部分给定的解题思路输出第一步。如果n不等于0，你需要按照已有步骤的思路和意见部分给出的提示，输出完整，清晰的下一步骤(第n+1步)。如果意见部分为空，就按照已有步骤的思路直接输出下一步。
输出格式限定为:
”下一步:...“
